document.addEventListener('DOMContentLoaded', () => {
    ...
    // 5. Iniciar Ronda (Prepara, no pide permiso aún)
    ...
    // 7. Botón "ESTOY LISTO" -> PEDIMOS EL PERMISO DEL SENSOR DE MOVIMIENTO
    document.getElementById('btn-empezar-turno').addEventListener('click', () => {
        
        // Atención: Ahora pedimos permiso para DeviceMotionEvent (Acelerómetro)
        if (typeof DeviceMotionEvent !== 'undefined' && typeof DeviceMotionEvent.requestPermission === 'function') {
            DeviceMotionEvent.requestPermission()
                ...
                .catch(error => {
                    console.error(error);
                    alert("Error pidiendo permiso. Recordá que debés subir la página a Render para que tenga conexión segura (HTTPS).");
                });
        } else {
            iniciarCuentaRegresiva();
        }
    ...
    // Función centralizada para procesar la acción y cambiar colores
    ...
    // ==========================================
    // NUEVA MATEMÁTICA: ACELERÓMETRO (Eje Z)
    // ==========================================
    window.addEventListener('devicemotion', (evento) => {
        if (!jugando || accionActivada) return;

        // La gravedad en el eje Z detecta perfectamente el cabeceo cuando está en la frente.
        // Estando vertical en tu frente (ángulo 0 que mencionaste), este valor ronda el 0.
        ...
        // Si la gravedad en Z cae por debajo de -4.5, significa que la pantalla mira al piso.
        if (gravedadZ < -4.5) { 
            procesarAccion('correcto', palabraPantalla);
            
        // Si sube de +4.5, la pantalla mira al techo.
